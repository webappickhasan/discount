...
const SinglePageModal = ( { open, setOpen } ) => {
	const dispatch = useDispatch();
	const { design_blocks } = useSelector( ( state ) => state.discount );
	const handleSinglePageTextChange = ( e ) => {
		const { value, name } = e.target;
		dispatch( updateSinglePage( { name, value } ) );
	};

	const handleTextColorChange = ( colors ) => {
		dispatch(
			updateSinglePage( { name: 'text_color', value: colors.hex } )
		);
	};
	const handleBgColorChange = ( colors ) => {
		dispatch( updateSinglePage( { name: 'bg_color', value: colors.hex } ) );
	};
	return (
